workflow StopVMByName
{
    ...
    foreach ($VMName in $VMNames) 
    {
        ...
        if($VMS)
        {
            ...
            {
                   Stop-AzureVM -ServiceName $VMS.ServiceName -Name $VMS.Name -Force
                  
                  #输出StartVM的UTC+8时间
                  $Start = [System.TimeZoneInfo]::ConvertTimefromUTC((get-date).ToUniversalTime(),$ChinaTimeZone)   
                  $Start.ToString("HH:mm:ss.ffffzzz") + " Stop VM : Service Name " +  $VMS.ServiceName + " VM Name " + $VMS.Name 
